...
function help() {
  echo "lenic-alpha $VERSION"
  echo "Usage: 'lenic-alpha [command]'"; echo ""
...
function install() {
  if [ "$ANSWER2" = "help" ]; then
    echo "lenic-alpha $VERSION - install tool"
    echo "Usage: 'lenic-alpha install [set of packages]'"; echo ""
...
function update() {
  echo "Get security updates and some other updates for your system."
...
function upgrade() {
  echo "upgrade tool in progress as of 2022-11-09"
}
...
function about() {
  ...
  echo "  / /  ___ ___  (_)___"
  echo " / /__/ -_) _ \/ / __/"
  echo "/____/\__/_//_/_/\__/   alpha"
  echo ""
  echo "Lenic alpha version $VERSION - DEB-Package"
...
function checkinput() {
  if [ "$ANSWER1" = "help" ]; then
    ...
    echo "Try 'lenic-alpha help' for more information!"
  fi	
}
